		...
		Pass
		{
			...
			CGPROGRAM
		        ...
		        struct VS_INPUT {
					...
					float4 uv_MainTex : TEXCOORD0; // index, is master bar, filtered, not used
				...
				v2g VS_Main(VS_INPUT v)
				{
                    ...
                    // Lerp position values for animations
					...
					o.normal = float3(idx, 0, isFiltered);
					o.color =  v.color;
				...
				void GS_Main(point v2g p[1], inout TriangleStream<g2f> triStream)
				{
					...
					if (p[0].isDiscarding < 0.9)
					{
						...
						float Depth = UNITY_ACCESS_INSTANCED_PROP(Props, _Depth);
						...
						float3 NEU = float3(Width,  0, Depth);
						float3 NED = float3(Width,  0, Depth);
						float3 NWU = float3(-Width, 0, Depth);
						float3 NWD = float3(-Width, 0, Depth);
						float3 SEU = float3(Width,  0, -Depth);
						float3 SED = float3(Width,  0, -Depth);
						float3 SWU = float3(-Width, 0, -Depth);
						float3 SWD = float3(-Width, 0, -Depth);
